...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/29 15:32:00 by achobany          #+#    #+#             */
/*   Updated: 2021/03/29 15:43:16 by achobany         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		work_with_flags(const char *arr, int i, va_list arg, t_option *flags)
{
	while (arr[i])
	{
		if (arr[i] != '-' && arr[i] != '0' && arr[i] != '.'
				&& arr[i] != '*' && !ft_isdigit(arr[i]) && !ft_istype(arr[i]))
			break ;
